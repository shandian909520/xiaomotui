#!/bin/bash

# 快速运行API测试脚本

echo "========================================"
echo "小磨推API测试执行"
echo "========================================"
echo ""

cd "$(dirname "$0")/.."

# 检查参数
if [ -z "$1" ]; then
    echo "运行所有API测试..."
    ./vendor/bin/phpunit tests/api --testdox
elif [ "$1" == "auth" ]; then
    echo "运行认证接口测试..."
    ./vendor/bin/phpunit tests/api/AuthTest.php --testdox
elif [ "$1" == "nfc" ]; then
    echo "运行NFC接口测试..."
    ./vendor/bin/phpunit tests/api/NfcTest.php --testdox
elif [ "$1" == "content" ]; then
    echo "运行内容接口测试..."
    ./vendor/bin/phpunit tests/api/ContentTest.php --testdox
elif [ "$1" == "coverage" ]; then
    echo "生成测试覆盖率报告..."
    ./vendor/bin/phpunit --coverage-html tests/coverage
    echo ""
    echo "覆盖率报告已生成到: tests/coverage/index.html"
    if command -v xdg-open &> /dev/null; then
        xdg-open tests/coverage/index.html
    elif command -v open &> /dev/null; then
        open tests/coverage/index.html
    fi
elif [ "$1" == "verbose" ]; then
    echo "运行详细测试..."
    ./vendor/bin/phpunit tests/api --verbose
else
    echo "运行指定测试: $1"
    ./vendor/bin/phpunit --filter "$1"
fi

echo ""
echo "========================================"
echo "测试完成"
echo "========================================"
echo ""
echo "使用说明:"
echo "  ./run_tests.sh          - 运行所有API测试"
echo "  ./run_tests.sh auth     - 运行认证测试"
echo "  ./run_tests.sh nfc      - 运行NFC测试"
echo "  ./run_tests.sh content  - 运行内容测试"
echo "  ./run_tests.sh coverage - 生成覆盖率报告"
echo "  ./run_tests.sh verbose  - 显示详细信息"
echo "  ./run_tests.sh [name]   - 运行指定测试"
echo ""
